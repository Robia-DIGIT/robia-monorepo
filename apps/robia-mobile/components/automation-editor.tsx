...
const TEMPLATE_FIELDS: Record<string, { key: string; label: string }[]> = {
  audit_completed: [],
  weekly_opportunities_summary: [{ key: 'organizationName', label: 'Nom de l?entreprise' }, { key: 'openOpportunityCount', label: 'Nombre d?opportunit?s annonc?' }],
...
export function AutomationEditor({ automation, onSaved }: { automation?: Automation; onSaved(a: Automation): Promise<unknown> }) {
  ...
  return <RobiaCard style={s.stack}><Text style={s.title}>{automation ? 'Modifier l?automatisation' : 'Cr?er une automatisation'}</Text>
    ...
    <Text style={s.title}>?tapes ? {steps.length}/20</Text>
    {steps.map((step,i) => <View key={i} style={s.stack}><Text style={s.body}>{i+1}. {ACTIONS.find(a => a.value === step.actionType)?.label ?? step.actionType}</Text>
      ...
      <AsyncButton label="Monter cette ?tape" disabled={i === 0} action={async () => setSteps(current => { const next = [...current]; [next[i-1], next[i]] = [next[i], next[i-1]]; return next; })} /><AsyncButton label="Retirer cette ?tape" action={async () => setSteps(current => current.filter((_,j) => i !== j))} />
    </View>)}
    <Choices value={action} onChange={setAction} options={ACTIONS} />
    {action === 'robia.action_items.create_internal_task' ? <Field label="Titre de la t?che" value={title} onChangeText={setTitle} /> : null}
    {trigger === 'event' && (needsAudit || needsApplication) ? <Toggle label="Utiliser le dossier ou l?audit de l??v?nement" value={useEvent} onChange={setUseEvent} /> : null}
    {action === 'robia.audit.run_diagnostic' || (needsAudit && !eventInput) ? <><Text style={s.body}>Site concern?</Text><Choices value={siteId} onChange={v => { setSiteId(v); setAuditId(''); }} options={websites.map(w => ({ value: w.id, label: w.domain ?? w.url }))} /></> : null}
    ...
    {action === 'robia.notification.send_email' ? <><Text style={s.body}>Cet e-mail est adress? au cr?ateur de l?automatisation. Les invitations aux candidats se g?rent dans le programme.</Text>
      ...
      {template !== 'audit_completed' ? <Text style={s.body}>Les valeurs ci-dessus sont enregistr?es telles quelles dans le mod?le d?e-mail.</Text> : null}
    </> : null}
    <AsyncButton label="Ajouter cette ?tape" disabled={steps.length >= 20} action={async () => {
      const input: Record<string,unknown> = {};
      if (action === 'robia.audit.run_diagnostic') { if (!siteId) throw new Error('Choisissez un site.'); input.websiteId = siteId; }
      if (needsAudit) { if (!eventInput && !auditId) throw new Error('Choisissez un audit termin?.'); input.auditId = eventInput ? '{{event.auditId}}' : auditId; }
      ...
      if (action === 'robia.notification.send_email') {
        ...
        for (const f of TEMPLATE_FIELDS[template]) { const value = templateData[f.key]?.trim(); if (!value || value.length > 200 || /[\r\n]/.test(value)) throw new Error(f.label + ' : saisissez une valeur sur une ligne (200 caract?res maximum).'); data[f.key] = value; }
        input.templateKey = template; input.templateData = data;
      }
      setSteps(current => [...current, { actionType: action, input }]);
    }} />
    <Toggle label="Approbation avant chaque ex?cution" value={approval} onChange={setApproval} />
    <Text style={s.body}>{automation ? 'L?enregistrement conserve l??tat actif ou inactif de cette automatisation.' : 'L?automatisation sera cr??e d?sactiv?e. Vous pourrez v?rifier ses ?tapes avant de l?activer.'}</Text>
