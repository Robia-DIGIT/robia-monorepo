...
export function Metric({ label, value }: { label: string; value?: number | string | null }) { const { compact } = useResponsiveLayout(); return <View style={[styles.row, compact && { flexDirection: 'column', alignItems: 'stretch', gap: 4 }]}><Text style={[s.body, { flex: 1 }]}>{label}</Text><Text style={[s.title, { flexShrink: 1 }]}>{value ?? "—"}</Text></View>; }
