...
export default function ProfileScreen() {
  ...
  const initials = (user?.name ?? organization?.name ?? "R")
    .split(" ")
    .slice(0, 2)
    .map((part) => part[0])
    .join("")
  ...
  return (
    <RobiaScreen fixedHeader>
      <View style={styles.topBar}>
        ...
        <Text accessibilityRole="header" style={styles.pageTitle}>Entreprise</Text>
        <Pressable
      ...
      <View
        ...
        <Text style={styles.name}>
          {user?.name ?? organization?.name ?? "Mon entreprise"}
        </Text>
        <Text style={styles.role}>
          {organization?.name ?? user?.company ?? "Espace ROBIA"}
        </Text>
      </View>
