...
export default function OpportunityScreen() {
  ...
  return (
    <RobiaScreen fixedHeader>
      <RobiaHeader compact back title="Opportunité" />
      <LoadState {...resource} retry={resource.reload} />
      {resource.data ? (
        <RobiaCard style={s.stack}>
          <Text style={s.title}>{resource.data.title}</Text>
          <Text style={s.body}>{resource.data.description}</Text>
          <Text style={s.body}>
            Impact : {resource.data.impactScore}/10 · Effort :{" "}
            {resource.data.effortScore}/10
          </Text>
          <Choices value={type} onChange={setType} options={DOCUMENT_TYPES} />
          <AsyncButton
            label="Générer ce contenu"
            action={async () => {
              const doc = await request<RobiaDocument>("/documents/generate", {
                method: "POST",
                body: { opportunityId: id, type },
                timeoutMs: 180000,
              });
              await refresh();
              router.push({ pathname: "/document", params: { id: doc.id } });
            }}
          />
          <AsyncButton
            label="Ajouter au plan d’action"
            action={() => generateActions(id)}
            onSuccess="Actions ajoutées au plan."
          />
          ...
          <AsyncButton
            ...
            disabled={status === resource.data.status}
            action={async () => {
              await request(
