...
function LocationDetail({ id, onRemoved }: { id: string; onRemoved(): Promise<unknown> }) {
  ...
  const weather = useResource<Weather>(
    hasCoordinates ? "/locations/" + encodeURIComponent(id) + "/weather" : null,
  );
  ...
  return (
    <RobiaCard style={s.stack}>
      ...
      <Text style={s.body}>
        {[r.data?.address, r.data?.city, r.data?.country]
          .filter(Boolean)
          .join(", ")}
      </Text>
      ...
      {r.data && !hasCoordinates ? (
        <Text style={s.body}>
          La météo sera disponible pour un établissement enregistré avec ses
          coordonnées Google Places.
        </Text>
      ) : null}
...
export default function LocationsScreen() {
  const { request } = useSession();
  const list = useResource<Location[]>("/locations");
  ...
  return (
    <RobiaScreen fixedHeader>
      ...
      <RobiaCard style={s.stack}>
        ...
        {results?.length === 0 ? (
          <Text style={s.body}>
            Aucun résultat. Vous pouvez saisir les informations ci-dessous.
          </Text>
        ) : null}
        {results?.map((c) => (
          <AsyncButton
            key={c.placeId}
            label={c.name + " · " + c.formattedAddress}
            action={async () => {
        ...
        <AsyncButton
          ...
          disabled={!name.trim()}
          action={async () => {
            const location = await request<Location>("/locations", {
              method: "POST",
              body: {
                name: name.trim(),
                placeId,
