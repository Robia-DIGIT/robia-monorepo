...
const metrics: Record<string,string> = { impressions: 'Vues', calls: 'Clics pour appeler', websiteClicks: 'Clics vers le site', directionRequests: 'Demandes d’itinéraire', conversations: 'Conversations' };
...
function LocationDetail({ location, reload }: { location: Location; reload(): Promise<unknown> }) {
  ...
  return <View style={s.stack}><Choices value={section} onChange={setSection} options={[{ value: 'info', label: 'Fiche' }, { value: 'reviews', label: 'Avis' }, { value: 'performance', label: 'Performances' }]} />
    {section === 'info' ? <><Text style={s.body}>{location.description || 'Aucune description renseignée.'}</Text><Text style={s.body}>{[location.primaryCategory, ...location.additionalCategories].filter(Boolean).join(" · ")}</Text>
      ...
      <Text style={s.body}>{{ OPEN: 'Ouvert', CLOSED_PERMANENTLY: 'Fermé définitivement', CLOSED_TEMPORARILY: 'Fermé temporairement' }[location.openStatus ?? ''] ?? 'Horaires non confirmés'}</Text>
      ...
      <Text style={s.title}>établissement ROBIA associé</Text><Text style={s.body}>{location.robiaLocation?.name ?? 'Aucun établissement associé'}</Text><LoadState {...local} retry={local.reload} />
    ...
    {section === 'reviews' ? <><AsyncButton label="Synchroniser les avis Google" action={async () => { await request(path + '/reviews/sync', { method: 'POST', timeoutMs: 90000 }); await reviews.reload(); }} /><LoadState {...reviews} retry={reviews.reload} empty={!reviews.data?.reviews.length} />
      ...
      {reviews.data?.reviews.map(r => <RobiaCard key={r.id} style={s.stack}><Text style={s.title}>{r.reviewerDisplayName || 'Client Google'} · {r.starRating ?? "—"}/5</Text><Text style={s.body}>{dateLabel(r.createTime)}</Text><Text style={s.body}>{r.comment || 'Avis sans commentaire'}</Text>{r.replyComment ? <Text style={s.body}>Réponse de l’établissement : {r.replyComment}</Text> : null}</RobiaCard>)}
    </> : null}
    {section === 'performance' ? <><Text style={s.body}>Interactions des 30 derniers jours disponibles. Google limite les actualisations à une par minute.</Text><AsyncButton label="Charger les performances" action={async () => setPerformance(await request<Performance>(path + '/performance', { timeoutMs: 90000 }))} />
...
export default function BusinessProfileScreen() {
  ...
  return <RobiaScreen fixedHeader><RobiaHeader compact back title="Fiches Google et avis" />
    <RobiaCard style={s.stack}><LoadState {...status} retry={status.reload} /><Text style={s.title}>Google Business Profile</Text><Text style={s.body}>{status.data?.connected ? status.data.googleAccountEmail : 'Connectez le compte qui gère vos Établissements.'}</Text>
      {status.data?.connected ? <><Status value={status.data.lastSyncStatus} /><Text style={s.body}>{status.data.locationCount} fiche(s) · {dateLabel(status.data.lastSyncedAt)}</Text>{status.data.stale || status.data.expired ? <Text style={s.body}>Les données doivent être actualisées.</Text> : null}
        ...
        <AsyncButton label="Déconnecter Google Business Profile" confirm="Retirer la connexion et les fiches Google synchronisées de ROBIA ?" action={async () => { await request(BASE, { method: 'DELETE' }); setSelected(null); await reload(); }} />
      ...
      <AsyncButton label="Vérifier la connexion" action={status.reload} />
    </RobiaCard>
