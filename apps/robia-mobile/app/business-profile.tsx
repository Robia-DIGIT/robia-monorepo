...
function LocationDetail({ location, reload }: { location: Location; reload(): Promise<unknown> }) {
  ...
  return <View style={s.stack}><Choices value={section} onChange={setSection} options={[{ value: 'info', label: 'Fiche' }, { value: 'reviews', label: 'Avis' }, { value: 'performance', label: 'Performances' }]} />
    {section === 'info' ? <><Text style={s.body}>{location.description || 'Aucune description renseign?e.'}</Text><Text style={s.body}>{[location.primaryCategory, ...location.additionalCategories].filter(Boolean).join(' ? ')}</Text>
      ...
      <Text style={s.title}>?tablissement ROBIA associ?</Text><Text style={s.body}>{location.robiaLocation?.name ?? 'Aucun ?tablissement associ?'}</Text><LoadState {...local} retry={local.reload} />
      <Choices value={link} onChange={setLink} options={local.data?.map(l => ({ value: l.id, label: l.name })) ?? []} />
      <AsyncButton label="Associer cet ?tablissement" disabled={!link || link === location.robiaLocationId} action={async () => { await request(path + '/link', { method: 'POST', body: { robiaLocationId: link } }); await reload(); }} />
      {location.robiaLocationId ? <AsyncButton label="Retirer l?association" confirm="Dissocier cette fiche Google de l??tablissement ROBIA ?" action={async () => { await request(path + '/link', { method: 'DELETE' }); setLink(''); await reload(); }} /> : null}
      <NavCard title="G?rer mes ?tablissements" description="Ajouter une adresse dans ROBIA." href="/locations" />
    </> : null}
    {section === 'reviews' ? <><AsyncButton label="Synchroniser les avis Google" action={async () => { await request(path + '/reviews/sync', { method: 'POST', timeoutMs: 90000 }); await reviews.reload(); }} /><LoadState {...reviews} retry={reviews.reload} empty={!reviews.data?.reviews.length} />
      {reviews.data ? <><Metric label="Note Google" value={reviews.data.averageRating == null ? null : reviews.data.averageRating + '/5'} /><Metric label="Nombre d?avis" value={reviews.data.totalReviewCount} /><Text style={s.body}>Synchronis? le {dateLabel(reviews.data.lastSyncedAt)}</Text></> : null}
      {reviews.data?.reviews.map(r => <RobiaCard key={r.id} style={s.stack}><Text style={s.title}>{r.reviewerDisplayName || 'Client Google'} ? {r.starRating ?? '?'}/5</Text><Text style={s.body}>{dateLabel(r.createTime)}</Text><Text style={s.body}>{r.comment || 'Avis sans commentaire'}</Text>{r.replyComment ? <Text style={s.body}>R?ponse de l??tablissement : {r.replyComment}</Text> : null}</RobiaCard>)}
    </> : null}
...
export default function BusinessProfileScreen() {
  ...
  return <RobiaScreen fixedHeader><RobiaHeader compact back title="Fiches Google et avis" />
    <RobiaCard style={s.stack}><LoadState {...status} retry={status.reload} /><Text style={s.title}>Google Business Profile</Text><Text style={s.body}>{status.data?.connected ? status.data.googleAccountEmail : 'Connectez le compte qui g?re vos ?tablissements.'}</Text>
      {status.data?.connected ? <><Status value={status.data.lastSyncStatus} /><Text style={s.body}>{status.data.locationCount} fiche(s) ? {dateLabel(status.data.lastSyncedAt)}</Text>{status.data.stale || status.data.expired ? <Text style={s.body}>Les donn?es doivent ?tre actualis?es.</Text> : null}
        ...
        <AsyncButton label="D?connecter Google Business Profile" confirm="Retirer la connexion et les fiches Google synchronis?es de ROBIA ?" action={async () => { await request(BASE, { method: 'DELETE' }); setSelected(null); await reload(); }} />
      </> : <><Text style={s.body}>La connexion s?effectue dans votre espace web ROBIA. Utilisez le m?me compte, puis revenez ici.</Text><AsyncButton label="Ouvrir les connexions ROBIA" action={async () => { await WebBrowser.openBrowserAsync('https://app.robiacopilot.site/analyse'); await status.reload(); }} /></>}
