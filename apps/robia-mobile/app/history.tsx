...
export default function HistoryScreen() {
    ...
    void (async () => {
      try {
        const groups = await Promise.all(
          websites.map((site) =>
            request<Audit[]>(`/audits${queryString({ website_id: site.id })}`),
          ),
        );
        setAudits(
          groups
  ...
  return (
    <RobiaScreen>
      ...
      />
      {loading ? <ActivityIndicator color={Brand.teal} /> : null}
      {!loading && !audits.length ? (
        <RobiaCard>
          <Text style={robiaStyles.cardTitle}>Aucun audit</Text>
          <Text style={robiaStyles.body}>
            Votre historique se construira à chaque nouvelle analyse.
          </Text>
        </RobiaCard>
      ) : null}
      {audits.map((audit, index) => (
        <RobiaCard
          key={audit.id}
          style={styles.card}
          accent={index === 0 ? Brand.teal : undefined}
        >
          <View style={styles.timeline}>
            <View style={[styles.dot, index === 0 && styles.dotActive]} />
            {index < audits.length - 1 ? <View style={styles.line} /> : null}
          </View>
          <View style={styles.copy}>
            <Text style={robiaStyles.cardTitle}>
              {index === 0 ? "Dernier diagnostic" : "Audit du site"}
            </Text>
          ...
          <View style={styles.score}>
            <Text style={styles.scoreValue}>{audit.globalScore ?? "—"}</Text>
            <Text style={robiaStyles.caption}>/100</Text>
            <MaterialIcons
              name="chevron-right"
              size={20}
