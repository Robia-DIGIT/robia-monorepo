...
function OutreachQueue({ program, applications }: { program: Program; applications: Application[] }) {
  ...
  return <View style={s.stack}><RobiaCard style={s.stack}><Text style={s.title}>Préparer les invitations</Text><Text style={s.body}>Sélectionnez les candidats dans l’ordre souhaité. Chaque e-mail sera ensuite envoyé avec votre confirmation.</Text>
    ...
    {!eligible.length ? <Text style={s.body}>Les dossiers à examiner ou en liste d’attente, avec une adresse e-mail, peuvent être invités.</Text> : null}
    <AsyncButton label={'Préparer ' + selection.length + ' invitation(s)'} disabled={!selection.length || selection.length > 50 || r.loading || !!r.error} action={async () => { await request(path, { method: 'POST', body: { applicationIds: selection } }); setSelection([]); await r.reload(); }} />
    ...
    {r.data?.map(o => <RobiaCard key={o.id} style={s.stack}><Text style={s.title}>{o.sortOrder}. {o.applicantName}</Text><Status value={o.status} /><Text style={s.body}>{o.recipientMasked}</Text>{o.lastError ? <Text style={s.body}>{o.lastError}</Text> : null}{o.sentAt ? <Text style={s.body}>{dateLabel(o.sentAt)}</Text> : null}
      {o.isNext ? <><Text style={s.body}>Objet : Candidature « {program.name} » — prochaine étape. L’e-mail invite cette personne à poursuivre sa candidature.</Text><AsyncButton label="Envoyer cette invitation" confirm={"Envoyer l’invitation à " + o.applicantName + ' (' + o.recipientMasked + ') ?'} action={async () => { try { await request('/odc/outreach/' + encodeURIComponent(o.id) + '/send', { method: 'POST' }); } finally { await r.reload(); } }} /></> : null}
      {['queued','failed'].includes(o.status) ? <AsyncButton label="Passer cette invitation" confirm="Retirer cette invitation de la séquence d’envoi ?" action={async () => { await request('/odc/outreach/' + encodeURIComponent(o.id) + '/skip', { method: 'POST' }); await r.reload(); }} /> : null}
    </RobiaCard>)}
  </View>;
}
export default function ProgramScreen() {
  ...
  return <RobiaScreen fixedHeader><RobiaHeader compact back title={r.data?.name ?? 'Programme'} /><LoadState {...r} retry={r.reload} />
    {r.data ? <><RobiaCard style={s.stack}><Status value={r.data.status} /><Text style={s.body}>{r.data.description}</Text><Metric label="Dossiers" value={list.data?.length} /><Choices value={section} onChange={setSection} options={[{ value: 'applications', label: 'Dossiers' }, { value: 'new', label: 'Ajouter' }, { value: 'outreach', label: 'Invitations' }, { value: 'settings', label: 'Programme' }]} /></RobiaCard>
      {section === 'applications' ? <><Field label="Rechercher un candidat" value={search} onChangeText={setSearch} /><Choices value={filter} onChange={setFilter} options={[{ value: 'all', label: 'Tous' }, { value: 'in_review', label: "À examiner" }, { value: 'incomplete', label: "À compléter" }, { value: 'waitlisted', label: 'Liste d’attente' }, { value: 'accepted', label: 'Acceptés' }, { value: 'rejected', label: 'Refusés' }]} /><Text style={s.body}>Classés par score final, puis par date.</Text><LoadState {...list} retry={list.reload} empty={!visible?.length} />
