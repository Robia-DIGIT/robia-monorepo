...
function DeliveryDetail({ id, refresh }: { id: string; refresh(): Promise<unknown> }) {
  ...
  const d = r.data;
  return <RobiaCard style={s.stack}><Text style={s.title}>Détail de l’envoi</Text><LoadState {...r} retry={r.reload} />{d ? <><Status value={d.status} /><Text style={s.body}>{d.recipientMasked} · {d.attemptCount} tentative(s)</Text><Text style={s.body}>Créé le {dateLabel(d.createdAt)}</Text><Text style={s.body}>{d.sentAt ? 'Envoyé le ' + dateLabel(d.sentAt) : 'Prochaine tentative prévue : ' + dateLabel(d.nextAttemptAt)}</Text>{d.lastError ? <Text style={s.body}>{d.lastError}</Text> : null}
