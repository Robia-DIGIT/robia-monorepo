...
export default function BillingScreen() {
  ...
  const r = useResource<Subscription>(
    organization ? "/billing/subscription" : null,
  );
  async function open(path: string, body?: unknown) {
    const session = await request<{ url: string }>(path, {
      method: "POST",
      body,
    });
  ...
  return (
    <RobiaScreen fixedHeader>
      <RobiaHeader compact back title="Mon abonnement" />
      ...
      {r.data ? (
        <RobiaCard style={s.stack}>
          <Text style={s.title}>Offre {r.data.plan}</Text>
          <Text style={s.body}>
            Statut :{" "}
            {{
              active: "Actif",
          ...
          {r.data.currentPeriodEnd ? (
            <Text style={s.body}>
              {r.data.cancelAtPeriodEnd ? "Fin prévue" : "Fin de période"} :{" "}
              {new Date(r.data.currentPeriodEnd).toLocaleDateString("fr-FR")}
            </Text>
          ) : null}
          <Text style={s.body}>
            Le prix et les conditions sont présentés sur la page sécurisée avant
            toute confirmation. Fermez le navigateur pour revenir dans RobIA.
          </Text>
          {r.data.canManage ? (
            <AsyncButton
              label="Gérer mon abonnement"
              action={() => open("/billing/portal-session")}
            />
          ) : null}
          {!["active", "trialing", "past_due", "incomplete"].includes(
            ...
            <>
              <AsyncButton
                label="Découvrir l’offre mensuelle"
                action={() =>
                  open("/billing/checkout-session", {
                    billingPeriod: "monthly",
                  })
                }
              />
              <AsyncButton
                label="Découvrir l’offre annuelle"
                action={() =>
                  open("/billing/checkout-session", { billingPeriod: "annual" })
                }
              />
            </>
          ) : null}
