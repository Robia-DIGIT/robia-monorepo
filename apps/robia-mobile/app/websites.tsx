...
export default function WebsitesScreen() {
  ...
  const { selectWebsite, selectedWebsiteId, refresh } = useRobiaData();
  const list = useResource<Site[]>(
    organization ? "/websites?include_archived=true" : null,
  );
    ...
    <RobiaScreen fixedHeader>
      ...
      {!organization ? (
        ...
        <>
          <RobiaCard style={s.stack}>
            ...
            <AsyncButton icon="integrations"
              ...
              action={async () => {
                ...
                const site = await request<Site>("/websites", {
                  method: "POST",
                  body: { url: normalized },
                });
                setUrl("");
                selectWebsite(site.id);
                await reload();
              }}
            />
          </RobiaCard>
          ...
          {list.data?.map((site) => (
            <RobiaCard key={site.id} style={s.stack}>
              <Text style={s.title}>{site.domain ?? site.url}</Text>
              ...
              {!site.archivedAt ? (
                  ...
                  <AsyncButton icon="website"
                    ...
                    action={async () => {
                      const current = await request<Site>(
                        "/websites/" + encodeURIComponent(site.id),
                      );
                      selectWebsite(current.id);
                ...
                <AsyncButton icon="refresh"
                  ...
                  action={async () => {
                    await request(
                      "/websites/" + encodeURIComponent(site.id) + "/restore",
                      { method: "PATCH" },
                    );
                    await reload();
                  }}
