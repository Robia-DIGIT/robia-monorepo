...
function ScoreEditor({ criterion, line, application, reload }: { criterion: Criterion; line?: ScoreLine; application: Application; reload(): Promise<unknown> }) {
  ...
  return <RobiaCard style={s.stack}><Text style={s.title}>{criterion.label}</Text><Text style={s.body}>{criterion.description} ? /{criterion.maxPoints} ? coefficient {criterion.weight}{criterion.required ? ' ? obligatoire' : ''}</Text>
    <Metric label="Proposition enregistr?e" value={line?.proposedPoints} /><Metric label="Note finale enregistr?e" value={line?.finalPoints} />
    {line?.rationale ? <Text style={s.body}>{line.rationale}</Text> : null}
    {!terminal(application.status) ? <><Field label="Note propos?e" value={proposed} onChangeText={setProposed} keyboardType="number-pad" /><Field label="Justification de la proposition" value={rationale} onChangeText={setRationale} multiline /><AsyncButton label="Enregistrer la proposition" disabled={!proposed.trim()} action={async () => { await request(path + '/propose-scores', { method: 'POST', body: { scores: [{ criterionId: criterion.id, proposedPoints: integer(proposed, criterion.label, 0, criterion.maxPoints), rationale: rationale.trim(), proposedBy: 'reviewer' }] } }); await reload(); }} />
...
function ApplicationBody({ application: a, reload }: { application: Application; reload(): Promise<unknown> }) {
  ...
  return <>
    <RobiaCard style={s.stack}><Text style={s.title}>{a.applicant.displayName}</Text><Text style={s.body}>{a.program.name}</Text><Status value={a.status} /><Text selectable style={s.body}>{[a.applicant.email,a.applicant.phone].filter(Boolean).join(' ? ')}</Text>
      ...
      {missingLabels(a).length ? <Text style={s.body}>? compl?ter : {missingLabels(a).join(', ')}</Text> : null}
      {a.decisionReason ? <Text style={s.body}>Motif de la d?cision : {a.decisionReason}</Text> : null}
    ...
    {section === 'answers' ? <RobiaCard style={s.stack}>
      ...
      {!a.program.fields.length ? <Text style={s.body}>Ce programme ne demande pas de r?ponse compl?mentaire.</Text> : null}
      {canEditAnswers(a.status) ? <AsyncButton label="Enregistrer les r?ponses" onSuccess="R?ponses enregistr?es." action={async () => { await request(path, { method: 'PATCH', body: answersPayload(a.program.fields, answers) }); await reload(); }} /> : null}
      ...
      {a.status === 'incomplete' ? <AsyncButton label="Enregistrer et rev?rifier le dossier" action={async () => { await request(path, { method: 'PATCH', body: answersPayload(a.program.fields, answers) }); await task('robia.odc.flag_missing_documents'); }} /> : null}
    </RobiaCard> : null}
    {section === 'documents' ? <><Text style={s.body}>Chaque pi?ce peut contenir un fichier de 10 Mo maximum.</Text>{!a.program.docTypes.length ? <Text style={s.body}>Aucune pi?ce demand?e pour ce programme.</Text> : null}
      {a.program.docTypes.map(d => { const file = a.documents.find(item => item.documentTypeId === d.id); return <RobiaCard key={d.id} style={s.stack}><Text style={s.title}>{d.label}{d.required ? ' ? obligatoire' : ''}</Text><Text style={s.body}>{file?.originalName ?? 'Aucun fichier joint'}</Text>{file ? <><Status value={file.status} /><Text style={s.body}>{(file.sizeBytes / 1024).toFixed(0)} Ko</Text></> : null}
        ...
        {canUpload(a.status) ? <AsyncButton label={file?.status === 'received' ? 'Remplacer le fichier' : 'Joindre un fichier'} confirm={file?.status === 'received' ? 'Le nouveau fichier remplacera la pi?ce actuelle apr?s son transfert.' : undefined} action={async () => { if (await uploadApplicationDocument(request, a.id, d)) await reload(); }} /> : null}
      </RobiaCard>; })}
      {a.status === 'incomplete' ? <AsyncButton label="Rev?rifier les pi?ces re?ues" action={() => task('robia.odc.flag_missing_documents')} /> : null}
    </> : null}
    {section === 'review' ? <><RobiaCard style={s.stack}><Text style={s.title}>Synth?se du dossier</Text><Field label="Synth?se de travail" value={summary} onChangeText={setSummary} multiline editable={!terminal(a.status)} />
      {!terminal(a.status) ? <><AsyncButton label="Enregistrer la synth?se" disabled={!summary.trim()} action={async () => { await request(path + '/propose-summary', { method: 'POST', body: { summaryDraft: summary.trim() } }); await reload(); }} /><AsyncButton label="Pr?parer un r?sum? de compl?tude" confirm={summary ? 'Remplacer la synth?se par un r?sum? des champs et pi?ces re?us ?' : undefined} action={() => task('robia.odc.prepare_application_summary')} /><AsyncButton label="Cr?er une t?che de revue" onSuccess="T?che ajout?e au plan d?action." action={() => task('robia.odc.create_review_task')} /></> : null}
    ...
    {section === 'decision' ? <RobiaCard style={s.stack}>
      {canDecide(a.status) ? <><Text style={s.title}>D?cision humaine</Text>{a.program.requireDualReview ? <Text style={s.body}>Ce programme demande une double revue. V?rifiez avec votre ?quipe qu?elle a ?t? effectu?e.</Text> : null}
        ...
        <Field label="Motif de la d?cision (obligatoire)" value={reason} onChangeText={setReason} multiline />
        <AsyncButton label="Enregistrer cette d?cision" disabled={!reason.trim()} confirm={'Confirmer ? ' + statusLabel(decision) + ' ? pour ' + a.applicant.displayName + ' ?\n' + reason.trim()} action={async () => { await request(path + '/decide', { method: 'POST', body: { decision, decisionReason: reason.trim() } }); setReason(''); await reload(); }} />
      </> : <Text style={s.body}>{terminal(a.status) ? 'La d?cision est enregistr?e. Ce dossier est d?sormais en lecture seule.' : 'Le dossier doit ?tre complet et ? examiner avant une d?cision.'}</Text>}
      {!terminal(a.status) ? <><Field label="Motif du retrait" value={withdrawReason} onChangeText={setWithdrawReason} multiline /><AsyncButton label="Retirer la candidature" disabled={!withdrawReason.trim()} confirm={'Retirer d?finitivement le dossier de ' + a.applicant.displayName + ' ?'} action={async () => { await request(path + '/withdraw', { method: 'POST', body: { reason: withdrawReason.trim() } }); await reload(); }} /></> : null}
    </RobiaCard> : null}
