...
function ApplicationBody({ application: a, reload }: { application: Application; reload(): Promise<unknown> }) {
  ...
  return <>
    <RobiaCard style={s.stack}><Text style={s.title}>{a.applicant.displayName}</Text><Text style={s.body}>{a.program.name}</Text><Status value={a.status} /><Text selectable style={s.body}>{[a.applicant.email,a.applicant.phone].filter(Boolean).join(" · ")}</Text>
      <Metric label="Score proposé" value={a.proposedTotal} /><Metric label="Score final" value={a.finalTotal} />
      {missingLabels(a).length ? <Text style={s.body}>À compléter : {missingLabels(a).join(', ')}</Text> : null}
      {a.decisionReason ? <Text style={s.body}>Motif de la décision : {a.decisionReason}</Text> : null}
      <Choices value={section} onChange={setSection} options={[{ value: 'answers', label: 'Dossier' }, { value: 'documents', label: 'Pièces' }, { value: 'review', label: 'Évaluation' }, { value: 'decision', label: 'Décision' }, { value: 'history', label: 'Historique' }]} />
    </RobiaCard>
    {section === 'answers' ? <RobiaCard style={s.stack}>
      {a.program.fields.map(f => <View key={f.key} style={s.stack}>{f.fieldType === 'select' && canEditAnswers(a.status) ? <><Text style={s.body}>{f.label}{f.required ? ' *' : ''}</Text><Choices value={answers[f.key] ?? ''} onChange={v => setAnswers({ ...answers, [f.key]: v })} options={[{ value: '', label: 'Non renseigné' }, ...(Array.isArray(f.options) ? f.options.filter((v): v is string => typeof v === 'string').map(v => ({ value: v, label: v })) : [])]} /></> : <Field label={f.label + (f.required ? ' *' : '') + (f.fieldType === 'date' ? ' (AAAA-MM-JJ)' : '')} value={answers[f.key] ?? ''} onChangeText={v => setAnswers({ ...answers, [f.key]: v })} editable={canEditAnswers(a.status)} multiline={f.fieldType === 'longtext'} keyboardType={f.fieldType === 'number' ? 'decimal-pad' : 'default'} />}</View>)}
      {!a.program.fields.length ? <Text style={s.body}>Ce programme ne demande pas de réponse complémentaire.</Text> : null}
      {canEditAnswers(a.status) ? <AsyncButton label="Enregistrer les réponses" onSuccess="Réponses enregistrées." action={async () => { await request(path, { method: 'PATCH', body: answersPayload(a.program.fields, answers) }); await reload(); }} /> : null}
      {a.status === 'draft' ? <AsyncButton label="Soumettre le dossier" confirm="Enregistrer les réponses et soumettre le dossier à vérification ?" action={async () => { await request(path, { method: 'PATCH', body: answersPayload(a.program.fields, answers) }); await request(path + '/submit', { method: 'POST' }); await reload(); }} /> : null}
    ...
    {section === 'decision' ? <RobiaCard style={s.stack}>
      {canDecide(a.status) ? <><Text style={s.title}>Décision humaine</Text>{a.program.requireDualReview ? <Text style={s.body}>Ce programme demande une double revue. Vérifiez avec votre équipe qu’elle a été effectuée.</Text> : null}
      ...
      </> : <Text style={s.body}>{terminal(a.status) ? 'La décision est enregistrée. Ce dossier est désormais en lecture seule.' : "Le dossier doit être complet et à examiner avant une décision."}</Text>}
    ...
    {section === 'history' ? <><LoadState {...history} retry={history.reload} empty={!history.data?.length} />{history.data?.map(e => <RobiaCard key={e.id} style={s.stack}><Text style={s.title}>{{ submitted: 'Dossier soumis', screening_started: 'Vérification commencée', screening_passed: 'Dossier complet', screening_failed: 'Dossier incomplet', decided: 'Décision enregistrée', withdrawn: 'Candidature retirée', 'odc.outreach.queued': 'Invitation préparée', 'odc.outreach.sent': 'Invitation envoyée', 'odc.outreach.skipped': 'Invitation passée' }[e.eventType] ?? e.eventType}</Text><Text style={s.body}>{dateLabel(e.createdAt)} · {e.actorUserId ? 'Intervention humaine' : 'Vérification automatique'}</Text>{e.toStatus ? <Status value={e.toStatus} /> : null}{e.payload?.reason || e.payload?.decisionReason ? <Text style={s.body}>{e.payload.reason ?? e.payload.decisionReason}</Text> : null}</RobiaCard>)}</> : null}
  </>;
}
