...
function ApplicationBody({ application: a, reload }: { application: Application; reload(): Promise<unknown> }) {
  ...
  return <>
    ...
    {section === 'answers' ? <RobiaCard style={s.stack}>
      ...
      {a.status === 'draft' ? <AsyncButton label="Soumettre le dossier" confirm="Enregistrer les réponses et soumettre le dossier ? vérification ?" action={async () => { await request(path, { method: 'PATCH', body: answersPayload(a.program.fields, answers) }); await request(path + '/submit', { method: 'POST' }); await reload(); }} /> : null}
