...
export default function OpportunitiesScreen() {
  ...
  const visibleOpportunities = opportunities.filter((item) => {
    if (filter === 'high') return item.impactScore >= 7;
    if (filter === 'quick') return item.effortScore <= 4;
    return true;
  });
  ...
  return (
    <RobiaScreen>
      ...
      <View style={styles.filters}>
        ...
        <FilterChip label="Prioritaires" active={filter === 'high'} onPress={() => setFilter('high')} />
        <FilterChip label="Actions rapides" active={filter === 'quick'} onPress={() => setFilter('quick')} />
      </View>
      ...
      {visibleOpportunities.map((item, index) => <OpportunityCard key={item.id} item={item} index={index} busy={busyId === item.id} onAction={act} />)}
    </RobiaScreen>
  );
}
